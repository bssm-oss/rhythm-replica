...
- 오디오 재생 서비스와 오디오 시간 기준 게임 엔진
- 에디터 타임라인, 파형 뷰, 자동 저장, 테스트 플레이 핸드오프, 선택/복사/붙여넣기/Undo/Redo/줌/미니맵/드래그 편집
- 에디터용 현재 비트 표시, BPM/Offset 테스트용 beat/time 이동 도구, 차트 검증 패널
- YouTube 메타데이터 조회 + 썸네일 미리보기 + 외부 도구 기반 가져오기 UI, 진행률/취소, 저장 위치 동작 설정
- 설정 화면에서 키 바인딩, 판정 윈도우, 캐시 관리, 테마/볼륨/판정선 설정
- GitHub Actions / DMG / Homebrew Cask용 배포 스캐폴드
...
> 서명/공증 시크릿이 없는 환경에서 만든 릴리즈는 unsigned 상태일 수 있습니다. 이 경우 Finder의 보안 경고에 따라 한 번 허용해야 할 수 있습니다.
...
> 현재 Cask는 설치 후 quarantine 속성을 제거하도록 설정되어 있어, unsigned 릴리즈에서도 Homebrew 설치 직후 실행 경로를 단순화합니다.
