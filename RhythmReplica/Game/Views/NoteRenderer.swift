...
enum NoteRenderer {
    static func draw(note: Note, rect: CGRect, in laneWidth: CGFloat, yPosition: CGFloat) {
        let x = CGFloat(note.lane) * laneWidth + 8
        let width = laneWidth - 16
        let noteRect = CGRect(x: rect.minX + x, y: yPosition, width: width, height: note.type == .long ? 36 : 18)
        let color: NSColor
        switch note.type {
